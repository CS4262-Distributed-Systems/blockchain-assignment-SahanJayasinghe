...
public final class AssetTransfer implements ContractInterface {
    ...
    private static final String DT_STR_FORMAT = "yyyy_MM_dd__HH_mm_ss";
    private SimpleDateFormat dtFormatter = new SimpleDateFormat(DT_STR_FORMAT);
    ...
    /**
     * Duplicates an asset given by the id.
     *
     * @param ctx the transaction context
     * @param assetID the ID of the asset to be duplicated
     * @param owner the owner of the asset given by assetID
     * @return the duplicated asset object
     */
    @Transaction(intent = Transaction.TYPE.SUBMIT)
    public Asset DuplicateAsset(final Context ctx, final String assetID, final String owner) {
        ChaincodeStub stub = ctx.getStub();

        String duplicateId = "dupl_" + assetID + "_" + getCurrentDTStr() + "_" + new Random().nextInt(10000);
        
        Asset asset = ReadAsset(ctx, assetID);
        Asset duplAsset = CreateAsset(ctx, duplicateId, asset.getColor(), asset.getSize(), owner, asset.getAppraisedValue());

        return duplAsset;
    }

    /**
    * used to generate a unique assetId for the duplicate
    * @return current date time as a string
    */
    private String getCurrentDTStr() {
        Calendar cal = Calendar.getInstance();
        return dtFormatter.format(cal.getTime());
    }
}
